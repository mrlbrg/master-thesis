...
This way, our results are not biased by the specific implementation, optimizations, and hardware on which we run our experiments.
All experiments were conducted locally on an Apple MacBook Pro with the specification listed in \autoref{tab:hardware-specs}.
...
We use the hourly Pageview Wikipedia dataset from \textbf{from 1 October 2025 at 00:00 UTC} as our base dataset.
...
To populate the database, we insert all articles from the filtered dataset once.
We then run the workload on the indices.
...
\begin{itemize}
    ...
    \item The keys follow a Zipf-like distribution, i.e., a small number of articles are very popular and receive a large number of operations, while the majority of articles receive only a few.
...
We run the indices in isolation to focus on the performance of the indices themselves without the overhead of maintaining the table data.
...
The base B-tree in both indices produces a tree with 502 nodes in total.
...
We can see that in the 3B-tree index, the majority of page writes occur in the Delta Tree rather than its base B-Tree.
...
% Index as part of the database vs. index in isolation
...
Both indices produce the same number of B-tree pages in all settings.
...
\begin{enumerate}
  ...
  When constantly swapping pages in and out of memory, we move the page write from one tree to another.
  ...
  When moving page writes from one tree to another, we expect to see the same amount of page writes overall.
...
Only with effective page caching can we reduce total number of page writes.
...
Therefore, the 3B-tree does not incur any overhead compared to the B-tree except for tracking changes in the B-tree nodes.
...
For example, with a 1\% threshold, we can fit 100 pages' changes into a single 4 KB page.
At a 25\% threshold, we can only fit 4 pages worth of changes in the worst case.
...
\item \textbf{Less Likelihood of Batching Changes:}
...
This means that, at some point, $x$ B-tree nodes have been modified. 
...
This workload introduces a lot of modification activity per page; therefore, we expect to see a more drastic reduction.
...
A more sequential workload would increase the degree of change for pages that end up in the Delta Tree.
...
To repeat, read amplification describes the number of \ac{IO} operations we need to perform to answer a query (see \autoref{sec:read-amplification-background}).
...
With a write threshold of 5\% for example, the number of leaf nodes in the Delta Tree is around 5\% of the total nodes of the B-tree.
If 1\% of the B-tree nodes are inner nodes, we can assume that inner nodes are always cached in memory and therefore never need to store deltas in the Delta Tree.
...
Therefore, we can keep the Delta Tree small compared to the B-tree, allowing us to cache it effectively in memory and keep the Delta Tree page reads low.
...
To analyze the overall \ac{IO} performance, we have illustrated the total \ac{IO} operations, separated into page reads and writes, in \autoref{fig:total-io} for different update ratios in the workload.
We can see that the baseline B-tree performs the same number of reads across all workloads, since the workload size and the index size remain the same.
We only change the number of updates in the workload, which affects the number of writes.
...
Only in workloads where we have enough writes to compensate for the additional reads can we achieve a net reduction in \ac{IO} operations.
...
In a setting where random writes are significantly more expensive than random reads, as on the M1 Pro's SSD with 512 GB, we could see faster runs in the 3B-tree even in scenarios where we have more total \ac{IO} operations due to a high fraction of read operations but fewer write operations.
However, some SSDs can be faster in writing than in reading.
...
Firstly, we store the state of each entry in the B-tree node to track whether it has been modified.
...
With a 4 KB page and 170 entries per node, losing one entry means a loss of \textasciitilde0.6\% in fanout.
...
\begin{enumerate}
  ...
  Our Delta Tree implementation stores the updated entries; therefore, variable-sized entries lead to larger deltas, increasing the size of the Delta Tree further.
  ...
  Therefore, we have fewer pages with a modification degree below the write threshold of 5\%.
  This leads to fewer pages being buffered in the Delta Tree, and therefore, a smaller overall write reduction.
\end{enumerate}
...
To summarize, our method works with variable-sized keys as well, but the write reduction is smaller, mainly due to the larger modification degrees of pages.
...
\bottomrule
\end{tabular}
\caption{Distribution of B-tree pages by their modification degree (percentage intervals) when being evicted with a write threshold of 5\% for fixed-sized keys and variable-sized keys in a workload with 100\% insertions. While both have similar number of page evictions, the variable-sized keys have more variance in the modification degree and fewer pages with a degree of change below the write threshold of 5\%.}
